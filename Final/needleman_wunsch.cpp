...
void needleman_wunsch::pm_compare() {
	vector<string> cads1 = add_gab(cad1, cad2);
	gcad1 = cads1[0];
	gcad2 = cads1[1];
	vector<string> cads2 = max_2string(gcad1, gcad2);
	...
	score = matrix[matrix.size() - 1][matrix[0].size() - 1][0];
	f_best_cads();
	cout << "S1: " << best_cads[0].first.first << endl;
	cout << "S2: " << best_cads[0].first.second << endl;
	cout << "Score: " << matrix[matrix.size() - 1][matrix[0].size() - 1][0] << endl;
	cout << "Score with Penalty: " << best_cads[0].second << endl;
}

void needleman_wunsch::get_time() {
	chrono::time_point<chrono::system_clock> Fill_Start, Fill_End;
	vector<string> cads1 = add_gab(cad1, cad2);
	gcad1 = cads1[0];
	gcad2 = cads1[1];
	vector<string> cads2 = max_2string(gcad1, gcad2);
	gcad1 = cads2[0];
	gcad2 = cads2[1];
	cout << "Matrix Fill-up ..." << endl;
	Fill_Start = chrono::system_clock::now();
	vector<vector<array<int, 4>>> matrix = g_matrix();
	Fill_End = chrono::system_clock::now();
	cout << "Matrix Fill-up Complete. Time: " << chrono::duration_cast<chrono::nanoseconds>(Fill_End - Fill_Start).count() << "ns." << endl;
	chrono::time_point<chrono::system_clock> Backtrack_Start, Backtrack_End;
	cout << "Backtrack Processing ..." << endl;
	Backtrack_Start = chrono::system_clock::now();
	getCads(matrix);
	score = matrix[matrix.size() - 1][matrix[0].size() - 1][0];
	f_best_cads();
	Backtrack_End = chrono::system_clock::now();
	cout << "Backtrack Processing Complete. Time: " << chrono::duration_cast<chrono::nanoseconds>(Backtrack_End - Backtrack_Start).count() << "ns." << endl;
}
